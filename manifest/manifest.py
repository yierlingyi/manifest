...
class Single(object):
  ...
  def hash(self):
    hasher = hashlib.md5()
    with open('/www/wwwroot/1201lzy.website/wp-content/themes/sakura-3.4.0/manifest/gallary/' + self.file, 'rb') as afile:
      buf = afile.read()
      hasher.update(buf)
  ...
  def optimize(self):
    im = Image.open('/www/wwwroot/1201lzy.website/wp-content/themes/sakura-3.4.0/manifest/gallary/' + self.file).convert('RGB')
...
def gen_manifest_json():
  onlyfiles = [f for f in os.listdir('/www/wwwroot/1201lzy.website/wp-content/themes/sakura-3.4.0/manifest/gallary/') if os.path.isfile(os.path.join('/www/wwwroot/1201lzy.website/wp-content/themes/sakura-3.4.0/manifest/gallary/', f))]
  id = 1
  Manifest = {}
  for f in onlyfiles:
    try:
